c*******************************************
      ...
      common/SERVICE/iseed_fromC,iPythDecay,parPYTH(100),charm
...
* set input PYTHIA parameters for Pro-Q2O tune 
      mstp(51)=7                 ! PDF set                                      
...
* set input PYTHIA parameters from SERVICE common:
      PARP(2)=5.d0              ! minimum c.m.s. energy of pp collision  
      MSTJ(22)=2                ! particle decays if lifetime < parj(71)
      PARJ(71)=10.              ! ctau=10 mm 
      CKIN(3)=ptmin               ! minimum pt in initial hard scattering, GeV 
      MSEL=parPYTH(1)                      ! QCD-dijet production 
      MSTU(21)=int(parPYTH(2))                  ! avoid stopping run 
      PARU(14)=int(parPYTH(3))              ! tolerance parameter to adjust fragmentation 
      MSTP(81)=int(parPYTH(4))                  ! pp multiple scattering off 
      MSTJ(21)=int(parPYTH(5))                    ! hadron decays off     
      MSTP (2)=int(parPYTH(6))                !which order running alphaS
      MSTP (33)=int(parPYTH(7))               !unclusion of k factor in cross section
      MSTP (51)=int(parPYTH(8))               !structure function chosen - CTEQ5M pdf
      MSTP (82)=int(parPYTH(9))               !Defines the multi-parton model
      PARP (67)=parPYTH(10)              !amount of initial-state radiation
      PARP (82)=parPYTH(11)              !pt cutoff for multiparton interactions
      MSTJ (11)=parPYTH(12)               !Choice of the fragmentation function
                       
* flags to select dijet production channels (active if msel=0)
      MSUB(11)=int(parPYTH(14))   ! q+q->q+q
      MSUB(12)=int(parPYTH(15))   ! q+qbar->q+qbar
      MSUB(13)=int(parPYTH(16))   ! q+qbar->g+g
      MSUB(28)=int(parPYTH(17))   ! q+g->q+g
      MSUB(53)=int(parPYTH(18))   ! g+g->q+qbar
      MSUB(68)=int(parPYTH(19))   ! g+g->g+g
                       
* flags to select prompt photon production channels (active if msel=0)
      MSUB(14)=int(parPYTH(21))   ! q+qbar->g+gamma
      MSUB(18)=int(parPYTH(22))   ! q+qbar->gamma+gamma
      MSUB(29)=int(parPYTH(23))   ! q+g->q+gamma
      MSUB(114)=int(parPYTH(24))  ! g+g->gamma+gamma
      MSUB(115)=int(parPYTH(25))  ! g+g->g+gamma
...
* creation of arrays for tabulation of nuclear overlap function
      ...
      do ib=1,110 
       ...
       write(*,*)'in HYINIT bmin', bmin,'bmax', bmax
