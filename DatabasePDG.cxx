...
Bool_t DatabasePDG::LoadParticles() {
  ifstream particleFile(fParticleFilename, ios::in); //AB: open in read-only mode
  if(!particleFile) {
  ...
  Char_t name[10]; //AB: increased by 1 (9 chars + 1 special schar
  ...
  while(1){ //AB: infinite loop

    if(particleFile.eof()) break; //AB: loop exit condition

    particleFile >> name >> mass >> width >> spin >> isospin >> isospinZ >> q >> s >> aq >> as >> c >> ac >> pdg;

    if(particleFile.eof()) break; //AB: second loop exit condition, to avoid empty string on some gcc version
...
Bool_t DatabasePDG::LoadDecays() {
  ...
  ifstream decayFile(fDecayFilename, ios::in); //AB: open in read-only mode
  ...
  while(1){ //AB: infinite loop
    if(decayFile.eof()) break; //AB: loop exit condition 
 
    //AB: new method for reading decay table
    decayFile >> mother_pdg;
    char tst1; decayFile.get(tst1); decayFile.putback(tst1); 
    while(tst1==' ') decayFile.get(tst1); decayFile.putback(tst1);

    for(int iDaughter=0; iDaughter<3; iDaughter++){
      char bufstr[100]={};
      daughter_pdg[iDaughter]=0;
      decayFile.get(bufstr,100,' ');

      for(int iBuf=strlen(bufstr)-1; iBuf>=0; iBuf--){
	if((bufstr[iBuf])=='.') iDaughter=3;
	decayFile.putback(bufstr[iBuf]);
      }
      if(iDaughter<3){
	decayFile >> daughter_pdg[iDaughter];
	char tst2; decayFile.get(tst2); decayFile.putback(tst2);
	while(tst2==' ') decayFile.get(tst2); decayFile.putback(tst2);
      }
    }

    decayFile >> branching;
    //AB: end of new method

    if(decayFile.eof()||(!decayFile.good())) break; //AB: second loop exit condition, to avoid empty string and infinite loop on some gcc version
	...
	for(Int_t kk=0; kk<nDaughters; kk++) 
	...
	if(!GetPDGParticle(daughter_pdg[kk])) {
	  ...
	  for(Int_t kkk=0; kkk<nDaughters; kkk++) 
...
Bool_t DatabasePDG::GetPDGParticleStatus(Int_t pdg) {
  ...
  if(nFindings == 0) {
    cout << "Warning in DatabasePDG::GetPDGParticleStatus(Int_t): The particle required with PDG = " << pdg
         << " was not found in the database!!" << endl;
    return kFALSE;
  }
  if(nFindings >= 2) {
...
Bool_t DatabasePDG::GetPDGParticleStatus(Char_t* name) {
  ...
  if(nFindings == 1) return fStatus[firstTimeIndex];
  if(nFindings == 0) {
    cout << "Warning in DatabasePDG::GetPDGParticleStatus(Char_t*): The particle required with name \"" << name
         << "\" was not found in the database!!" << endl;
    return kFALSE;
  }
  if(nFindings >= 2) {
